...
export function Component (): JSX.Element
{
  ...
  useEffect( () =>
  {
    const fetchData = async () =>
    {
      ...
      try
      {
        const response = await fetch( "http://192.168.86.26:3001/data" );
  ...
  const convertTimeSeriesToCSV = ( filteredData, formData ) =>
  {
    ...
    const endYear = formData.years[ 1 ] || Math.max( ...filteredData.map( item => item.year_old ) );
    ...
    // Aggregate data by year for the total count
    filteredData.forEach( item =>
    {
      countByYear[ item.year_old ]++;
    } );

    // Initialize and aggregate data by year for each country, if any
    const initialCountByYearAndCountry = {};
    formData.country.forEach( country =>
    {
      initialCountByYearAndCountry[ country ] = { ...countByYear }; // Initialize with zeros
      filteredData.forEach( item =>
      {
        if ( item.locationCountryNew.includes( country.toUpperCase() ) )
        {
    ...
    // Prepare CSV content
    let csvContent = "Year," + ( formData.country.length > 0 ? formData.country.join( "," ) + "," : "" ) + "Total\n";
    for ( let year = startYear; year <= endYear; year++ )
    {
      let row = `${ year }`;
      formData.country.forEach( country =>
      ...
      row += `,${ countByYear[ year ] }`; // Add total count
      csvContent += row + "\n";
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <MenuItem value="labor">Labor Unrests</MenuItem>
            <MenuItem value="unemployment">Unemployment Related</MenuItem>
            <MenuItem value="land">Land Related</MenuItem>
            <MenuItem value="student">Student/Youth</MenuItem>
            <MenuItem value="nationalist">Nationalist Movements</MenuItem>
            <MenuItem value="fascist">Fascist/Anti-Fascist</MenuItem>
            <MenuItem value="extremist">Extremist Movements</MenuItem>
          </Select>
        </FormControl>
